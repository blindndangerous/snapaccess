...
- **Deck selector and deck editor ping-ponged (Back needed twice).** The deck-selector tray matched on the shared `OverlayCanvas` root, so it also claimed the deck-editor screen and the two navigators alternated, forcing two Back presses. The tray now matches only its own view (`PlayDeckTrayView_Landscape`), so the selector and the editor are mutually exclusive and a single Back exits.
- **Trapped in the landscape deck editor.** The deck-editor detection only recognized the portrait `DeckListCardSlotView` layout, so on the landscape collection deck editor the deck navigator never activated — MainMenu stayed in control and the user was stuck cycling the menu bar inside the editor (force-quit required). The landscape editor (`DeckCardsView`) is now detected, so the navigator activates and Backspace closes it. The close-button search also now includes the game's generic back button (`btn_hex_prp` / `Escape_BackButton`).
...
- **"Edit Current Deck" dead-ended with "Edit button not found".** The Play screen has no edit button (the game only edits a deck through the deck selector), so the menu item failed outright. It now opens the deck selector and tells the user to pick a deck and press E to edit.
