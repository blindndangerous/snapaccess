...
- **Deck selector and deck editor ping-ponged (Back needed twice).** The deck-selector tray and the deck editor share the OverlayCanvas root, so both navigators claimed the editor screen and alternated, forcing two Back presses to exit. The tray now yields when the editor (`DeckCardsView`) is open, so one navigator is active and a single Back exits.
