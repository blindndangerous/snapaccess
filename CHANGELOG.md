...
- **Auto-Deck editor could not be exited by keyboard.** The deck editor's back button (`btn_hex_prp` under `Escape_BackButton`, the game's generic back button) wasn't in the close-button search, so the mod simulated Escape, which didn't close it, and the user was trapped (force-quit required). The real back button is now searched first.
